...
| 段 | 核心動作 |
|----|---------|
| **A** 前置 | eyes reaction → checkout(fetch-depth:0) → `resolve_branch`（找或建 `issue/{N}-*`）→ HTTPS git config → `save_sha` |
...
| **K** AI 驗收 | `detect_smoke` 檢查 diff 有無動到 `js/src/`、`inc/classes/`、`inc/templates/`、`inc/assets/` → 建置前端（`pnpm run build` 單一指令）→ 安裝 CJK 字型 → Playwright 裝 chromium → `run_ai_acceptance`（agent=`wp-workflows:browser-tester`） |
...
| Marketplace | `https://github.com/j7-dev/wp-workflows.git`（提供 4 個 agents：clarifier / planner / tdd-coordinator / browser-tester） |
